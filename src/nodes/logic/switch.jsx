...
export const switchNode = {
  ...
  renderHelp() {
    return (
      <>
        <p>Route messages based on property values. Each rule creates an output, and messages are sent to outputs whose rules match.</p>

        <h5>Comparison Types</h5>
        <ul>
          <li><code>==</code>, <code>!=</code>, <code>&lt;</code>, <code>&gt;</code>, <code>&lt;=</code>, <code>&gt;=</code> - Numeric/string comparisons</li>
          <li><strong>contains</strong> - Check if string contains substring</li>
          <li><strong>matches regex</strong> - Test against regular expression</li>
          <li><strong>is true</strong>, <strong>is false</strong> - Boolean checks</li>
          <li><strong>is null</strong>, <strong>is not null</strong> - Null checks</li>
          <li><strong>is of type</strong> - Check JavaScript type (string, number, object, etc.)</li>
          <li><strong>otherwise</strong> - Always matches (use as final catch-all)</li>
        </ul>

        <h5>Options</h5>
        <ul>
          <li><strong>Property</strong> - Which property to check (default: <code>msg.payload</code>)</li>
          <li><strong>checking all rules</strong> - If enabled, message can go to multiple outputs. If disabled, stops at first match.</li>
          <li><strong>recreate message sequences</strong> - Preserve message sequence parts info</li>
        </ul>

        <h5>Example</h5>
        <p>To route by status code:</p>
        <pre>{`Rule 1: payload.status == "success" → output 1
Rule 2: payload.status == "error" → output 2
Rule 3: otherwise → output 3`}</pre>
      </>
    );
  }
};
